...
from app.schemas.log_activity_schema import Log_Individu, Detail_Log
...
@log.get("/show", response_model=Page[Detail_Log], status_code = status.HTTP_200_OK)
async def show_log(otoriti: is_admin_depend, db: db_dependency, email: Optional[str] = None) -> Page[Detail_Log]:
    try:
        if otoriti:
            query = select(LogActivity)
            if email:
                query = query.where(LogActivity.email.ilike(f"%{email}%"))
            query = query.order_by(LogActivity.created_at.desc())
